...
const CashMemo = ({ className }) => {
  ...
  const handleInputChange = (index,field,value)=>{
    ...
    const quantity =
      parseFloat(convertBanglaToEnglish(updatedItems[index].quantity)) || 0;

    const rate =
      parseFloat(convertBanglaToEnglish(updatedItems[index].rate)) || 0;
  ...
  const handleKeyDown = (e,rowIndex)=>{
    ...
    if(e.key==='ArrowDown'){
      ...
      newActive[rowIndex] =
        Math.max(newActive[rowIndex]-1,-1);
      ...
      e.key==='Enter' &&
      activeSuggestionIndex[rowIndex] >= 0
    ){
      ...
      handleSuggestionClick(
        rowIndex,
        rowSuggestions[rowIndex][activeSuggestionIndex[rowIndex]]
      );
                      ...
                      <ul className="absolute z-10 bg-white border w-52 md:w-full max-h-48 overflow-y-auto mt-10 rounded-md shadow-lg">
                        ...
                        {rowSuggestions[index].map((sug,sugIndex)=>(
                          ...
                          <li
                            key={sug}
                            onClick={()=>
                              handleSuggestionClick(index,sug)
                            }
                            className={`px-4 py-2 cursor-pointer hover:bg-blue-50
                            ${sugIndex===activeSuggestionIndex[index]
